...
void CTriggerComponent::Initialize()
{
	...
	{
		// Always update the default listener to the camera/player position
		Matrix34 transformation = Matrix34::CreateTranslationMat(gEnv->pSystem->GetViewCamera().GetPosition());
		gEnv->pAudioSystem->GetListener()->SetTransformation(transformation);

		// Retrieve and log the listener's position
			Matrix34 listenerTransform = transformation;
		Vec3 listenerPos = listenerTransform.GetTranslation();
		//CryLogAlways("[Audio System] Listener position: x=%.2f y=%.2f z=%.2f", listenerPos.x, listenerPos.y, listenerPos.z);

		CryLogAlways("[Audio System] Listener Created!");
	}
	...
	if (m_pIEntityAudioComponent != nullptr)
	{
		...
		CryLogAlways("[Audio][TriggerComponent] Initialized with custom aux object id: %u", m_auxObjectId);
	}
	else
	{
		...
		CryLogAlways("[Audio][TriggerComponent] Created new IEntityAudioComponent, using default aux object id: %u", m_auxObjectId);
	}

	if (m_playTrigger.m_autoPlay && gEnv->IsEditor())
	{
		CryLogAlways("[Audio][TriggerComponent] AutoPlay enabled in editor, calling Play()");
		Play();
	}
...
void CTriggerComponent::ProcessEvent(const SEntityEvent& event)
{
	if (m_pIEntityAudioComponent != nullptr)
	{
		...
		case ENTITY_EVENT_AUDIO_TRIGGER_STARTED:
		{
			...
			CryLogAlways("[Audio][TriggerComponent] AUDIO_TRIGGER_STARTED. ActiveInstances: %d, AuxObjectId: %u", m_numActiveTriggerInstances, m_auxObjectId);
			break;
		}
		case ENTITY_EVENT_AUDIO_TRIGGER_ENDED:
		{
			...
			--m_numActiveTriggerInstances;
			CryLogAlways("[Audio][TriggerComponent] AUDIO_TRIGGER_ENDED. ActiveInstances: %d, AuxObjectId: %u", m_numActiveTriggerInstances, m_auxObjectId);
			...
			if (m_numActiveTriggerInstances == 0)
			{
				...
				CryLogAlways("[Audio][TriggerComponent] All trigger instances ended. Event AuxObjectId: %u", auxObjectId);
		...
		case ENTITY_EVENT_UPDATE:
			{
				...
				// Log the listener's position every frame
				if (gEnv->pAudioSystem)
				{
					auto* pListener = gEnv->pAudioSystem->GetListener();
					if (pListener)
					{
						// If GetTransformation() is available, use it. Otherwise, log the position you set.
						// Here, we assume you set the listener's transformation to the camera position.
						Vec3 listenerPos = gEnv->pSystem->GetViewCamera().GetPosition();
						//CryLogAlways("[Audio System] Listener position: x=%.2f y=%.2f z=%.2f", listenerPos.x, listenerPos.y, listenerPos.z);
					}
				}
...
{
	CryLogAlways("[Audio][TriggerComponent] Play() called. PlayTriggerId: %u, AuxObjectId: %u", m_playTrigger.m_id, m_auxObjectId);
	...
	if ((m_pIEntityAudioComponent != nullptr) && (m_playTrigger.m_id != CryAudio::InvalidControlId))
	{
		...
		CryLogAlways("[Audio][TriggerComponent] ExecuteTrigger called for PlayTriggerId: %u", m_playTrigger.m_id);
	}
	else
	{
		CryLogAlways("[Audio][TriggerComponent] Play() failed: AudioComponent or PlayTriggerId invalid.");
	}
}
...
{
	CryLogAlways("[Audio][TriggerComponent] Stop() called. StopTriggerId: %u, CanStop: %d, AuxObjectId: %u", m_stopTrigger.m_id, m_stopTrigger.m_canStop, m_auxObjectId);
	...
	if ((m_pIEntityAudioComponent != nullptr) && m_stopTrigger.m_canStop)
	{
		if (m_stopTrigger.m_id != CryAudio::InvalidControlId)
		{
			...
			CryLogAlways("[Audio][TriggerComponent] ExecuteTrigger called for StopTriggerId: %u", m_stopTrigger.m_id);
		}
		else if (m_playTrigger.m_id != CryAudio::InvalidControlId)
		{
			...
			CryLogAlways("[Audio][TriggerComponent] StopTrigger called for PlayTriggerId: %u", m_playTrigger.m_id);
		}
	}
	else
	{
		CryLogAlways("[Audio][TriggerComponent] Stop() failed: AudioComponent invalid or CanStop is false.");
	}
}
