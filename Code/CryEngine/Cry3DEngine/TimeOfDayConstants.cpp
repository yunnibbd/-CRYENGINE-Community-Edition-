...
void ImageSettingsImpl::ResetVariables()
{
	sharpening = 0.0f;
	texturesharpening = 0.0f;
	vibrance = 0.0f;


	if (ICVar* pSharpening = gEnv->pConsole->GetCVar("r_Sharpening"))
		pSharpening->Set(sharpening);

	if (ICVar* pTextureSharpening = gEnv->pConsole->GetCVar("r_TextureSharpening"))
		pTextureSharpening->Set(texturesharpening);
...
void ImageSettingsImpl::Serialize(Serialization::IArchive& ar)
{
	ar(yasli::Range(sharpening, 0.0f, 25.0f), "Sharpening", "Sharpening");
	ar(yasli::Range(texturesharpening, 0.0f, 25.0f), "TextureSharpening", "Texture Sharpening");
	ar(yasli::Range(vibrance, -1.0f, 2.0f), "Vibrance", "Vibrance");
...
void TotalIllumImpl::Serialize(Serialization::IArchive& ar)
{
	...
	SerializeSvoTi(ar, minNodeSize, 0.01f, 512.0f, "MinNodeSize", "Min Node Size", "e_svo");
