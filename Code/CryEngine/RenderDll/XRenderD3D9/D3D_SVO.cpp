...
void CSvoRenderer::RayTracedReflectionsPass(SSvoTargetsSet* pTS)
{
	...
	//SetupCommonConstants(pTS, rp, pTS->pRT_RT_REFL);
	//SetupRayTracingReflectionConstants(rp);

	//rp.Execute();
}

void CSvoRenderer::SetupRayTracingReflectionConstants(CSvoFullscreenPass& rp)
{
	static CCryNameR rtReflParamsName("SVO_RTReflParams");
	Vec4 rtReflParams(
		e_RT_Reflections_MaxDistance,
		e_RT_Reflections_Roughness,
		e_RT_Reflections_Samples,
		e_RT_Reflections // Use general RT active flag
	);

	rp.SetConstantArray(rtReflParamsName, (Vec4*)&rtReflParams, 1);
}
...
uint64 CSvoRenderer::GetRunTimeFlags(bool bDiffuseMode, bool bPixelShader)
{
	...
	if (e_svoTI_RT_Active && e_RT_Reflections && bPixelShader && !bDiffuseMode)
...
void CSvoRenderer::CheckCreateUpdateRT(_smart_ptr<CTexture>& pTex, int nWidth, int nHeight, ETEX_Format eTF, ETEX_Type eTT, int nTexFlags, const char* szName)
{
	...
	//if (e_svoTI_RT_Active && e_RT_Reflections)
	//{
	//	CheckCreateUpdateRT(pTS->pRT_RT_REFL, nWidth, nHeight, eTF_R16G16B16A16F, eTT_2D, FT_STATE_CLAMP, "SVO_RT_REFLECTIONS");
	//}
}
