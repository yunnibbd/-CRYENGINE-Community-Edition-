...
void CSceneDepthStage::ReadbackLatestData()
{
	...
	const auto readbackData = [&readback, &result](void* pData, uint32 rowPitch, uint32 slicePitch) -> bool
		{
			// CRITICAL: Add null pointer check to prevent shutdown crashes
			if (!pData)
			{
				CryLogAlways("CSceneDepthStage: Readback data is null - skipping depth readback");
				return false;
			}
			...
			// ENHANCED: Add additional safety checks for depth data processing
			if (readback.flags & kNativeDepth)
			{
				for (uint32 i = 0; i < CULL_SIZEX * CULL_SIZEY; ++i)
				{
					// SAFETY: Check for valid depth values before processing
					if (pDepths && std::isfinite(pDepths[i]))
					{
					...
					{
						// Use safe fallback value for invalid depth data
						result.data[i] = FLT_EPSILON;
					}
				}
			}
			else
			{
				// SAFETY: Check for valid projection parameters before division
				const float zFarMinusNear = readback.zFar - readback.zNear;
				const float zNearMinusFar = readback.zNear - readback.zFar;

				if (std::abs(zFarMinusNear) < FLT_EPSILON || std::abs(zNearMinusFar) < FLT_EPSILON)
				{
					CryLogAlways("CSceneDepthStage: Invalid projection parameters - using fallback values");
					// Use safe fallback values when projection parameters are invalid
					for (uint32 i = 0; i < CULL_SIZEX * CULL_SIZEY; ++i)
					{
						result.data[i] = FLT_EPSILON;
					}
				...
				{
					const float projRatioX = readback.zFar / zFarMinusNear;
					const float projRatioY = readback.zNear / zNearMinusFar;

					for (uint32 i = 0; i < CULL_SIZEX * CULL_SIZEY; ++i)
					{
						// SAFETY: Check for valid depth values and prevent division by zero
						if (pDepths && std::isfinite(pDepths[i]) && std::abs(pDepths[i]) >= FLT_EPSILON)
						{
							const float maxDepth = std::max(pDepths[i], FLT_EPSILON);
							const float calculatedDepth = projRatioY / maxDepth + projRatioX;
							result.data[i] = std::max(calculatedDepth, FLT_EPSILON);
						}
						else
						{
							// Use safe fallback value for invalid depth data
	...
	// SAFETY: Add try-catch around the staging resource access to prevent crashes
	try
	{
	...
	{
		CryLogAlways("CSceneDepthStage: Exception during staging resource access - skipping this readback");
		// Reset readback state on failure
		readback.bCompleted = false;
		readback.bReceived = false;
	}
}