...
{
	// Clamp shadow map sizes to avoid device/driver crashes on large allocations
	const int maxShadowMapSize = 4096; // Safe default, can be adjusted or queried from device caps
	...
	{
		int shadowPoolSize = shadowConfig.nPoolSize;
		if (shadowPoolSize > maxShadowMapSize)
		{
			CryLogAlways("Requested shadow pool size %d exceeds device limit %d. Clamping.", shadowPoolSize, maxShadowMapSize);
			shadowPoolSize = maxShadowMapSize;
		}

		if (!m_pTexRT_ShadowPool)
		{
			CryLogAlways("Error: m_pTexRT_ShadowPool is null during shadow resource allocation!");
			return;
		}
		...
		if (!CTexture::IsTextureExist(m_pTexRT_ShadowPool))
		{
			...
			if (!CTexture::IsTextureExist(m_pTexRT_ShadowPool))
			{
				CryLogAlways("Failed to create shadow pool texture (%d x %d, format %d)", shadowPoolSize, shadowPoolSize, (int)eShadowTexFormat);
				return;
			}
		}
	}
	...
	// =======================================
	// allocate shadow map cache textures
	{
		...
		for (int i = 0; i < MAX_GSM_LODS_NUM; ++i)
		{
			...
			int res = nResolutions[i];
			if (res > maxShadowMapSize)
			{
				CryLogAlways("Requested shadow map cache size %d for cascade %d exceeds device limit %d. Clamping.", res, i, maxShadowMapSize);
				res = maxShadowMapSize;
			}

			if (!pTx)
			{
				char szName[64];
				cry_sprintf(szName, "$ShadowMapCached_%d_%s", i, m_graphicsPipeline.GetUniqueIdentifierName().c_str());
				pTx = CTexture::GetOrCreateTextureObject(szName, res, res, 1, eTT_2D, FT_DONT_STREAM, texFormat);
			}

			// Check if the embedded device-resource matches our config and re-create it when it differs only
			pTx->Invalidate(res, res, texFormat);
			...
			if (!CTexture::IsTextureExist(pTx) && res > 0 && i < cachedCascadesCount)
			{
				const float mb = sqr(res) * CTexture::BitsPerPixel(texFormat) / (1024.f * 1024.f * 8.f);
				CryLog("Allocating shadow map cache %d x %d: %.2f MB", res, res, mb);
				pTx->CreateDepthStencil(texFormat, Clr_FarPlane);

				if (!CTexture::IsTextureExist(pTx))
				{
					CryLogAlways("Failed to allocate shadow map cache texture for cascade %d (%d x %d, format %d)", i, res, res, (int)texFormat);
					continue;
				}
	...
	for (int lightType = ePass_DirectionalLight; lightType <= ePass_DirectionalLightCached; ++lightType)
	{
		for (auto& cachedPass : m_ShadowMapPasses[lightType])
		{
				...
				cachedPass.m_eFrustumType == EFrustumType::e_Nearest)
			{
				...
				{
					int width = cachedPass.m_pDepthTarget->GetWidth();
					int height = cachedPass.m_pDepthTarget->GetHeight();

					if (width > maxShadowMapSize || height > maxShadowMapSize)
					{
						CryLogAlways("Requested dynamic shadow map size %dx%d exceeds device limit %d. Clamping.", width, height, maxShadowMapSize);
						width = height = maxShadowMapSize;
					}
					...
					if (!CTexture::IsTextureExist(cachedPass.m_pDepthTarget))
					{
						...
						if (!CTexture::IsTextureExist(cachedPass.m_pDepthTarget))
						{
							CryLogAlways("Failed to allocate dynamic shadow map for pass (type %d, %d x %d, format %d)", lightType, width, height, (int)texFormat);
						}
					}
				}
			}
	...
	// =======================================
	// ensure far plane texture is valid
	{
		...
		if (!CTexture::IsTextureExist(CRendererResources::s_ptexFarPlane))
		{
			...
			if (!CTexture::IsTextureExist(CRendererResources::s_ptexFarPlane))
			{
				CryLogAlways("Failed to create s_ptexFarPlane shadow texture!");
			}
			else
			{
