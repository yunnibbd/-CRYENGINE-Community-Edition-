...
bool CSharpening::Preprocess(const SRenderViewInfo& viewInfo)
{
	...
	// Pass is active if any control is active (including vibrance)
	if (
		fabs(m_pAmount->GetParam() - 1.0f) +
		fabs(CRenderer::CV_r_Sharpening) +
		fabs(CRenderer::CV_r_ChromaticAberration) +
		fabs(CRenderer::CV_r_Vibrance)
		> 0.09f)
	{
		return true;
	}
...
bool CVibrance::Preprocess(const SRenderViewInfo& viewInfo)
{
	bool bQualityCheck = CPostEffectsMgr::CheckPostProcessQuality(eRQ_Medium, eSQ_Medium);
	...
	if (fabs(m_pAmount->GetParam() - 1.0f) + CRenderer::CV_r_Vibrance)
	{
		return true;
	}
...
void CVibrance::Reset(bool bOnSpecChange)
{
	m_pAmount->ResetParam(1.0f);
	m_pType->ResetParam(0.0f);
}
